...
#https://www.ctolib.com/topics-119926.html
Python分布式爬虫打造搜索引擎
...
实现全部文章字段下载的代码：
    ...
    def parse(self, response):
        """ 1. 获取文章列表页中的文章url并交给scrapy下载后并进行解析 2. 获取下一页的url并交给scrapy进行下载， 下载完成后交给parse """
...
def parse(self, response):
    """ 提取出html页面中的所有url 并跟踪这些url进行一步爬取 如果提取的url中格式为 /question/xxx 就下载之后直接进入解析函数 """
...
copy.copy(r) for r in self.rules]将我们的rules进行一个copy
调用回调函数get_method。
