...
'limit':'25'
    ...
    url = 'http://www.gzmpc.com/gzmpcscm3/extjsgridQueryServlet/query?_dc='+str(times)+'&startIndex=0&pageRowNum=1000&needpagecount=&gridcode=func-salesflowsquery-grid&queryType=query&dataSource=salesflowsqueryquery&stagetype=SALESFLOWS&stageid=5&querymoduleid=FuncSalesflowsqueryNorQuery&sumfieldnames=goodsqty&fieldName_0=credate__2&opera_0=oper_equal&value1_0=2018-02-05%2023%3A59%3A59&fieldName_1=credate&opera_1=oper_between&value1_1=2018-01-26%2000%3A00%3A00&value2_1=2018-02-05%2023%3A59%3A59&oper_length=2&page=1&start=0&limit=25'    #改时间
...
def seles_dx(pl,email):
    ...
    for row in pl["rows"][0:-1]:   #[0:-1]除去最后合计项
       ...
       # print '--------------------------------------'
        rows=customname+','+goodsname+','+goodstype+','+salesdate+','+ goodsqty+','+ goodsunit+','+ lotno+','+unitprice+','+u'欣特'+'\n'
        try:
           with open('./lx/'+nemes,'a')as f:         #保存最后爬取的信息
               s=str(rows)
               f.write(s)
...
'limit':'25'
    ...
    url = 'http://www.gzmpc.com/gzmpcscm3/extjsgridQueryServlet/query?_dc='+str(times)+'&startIndex=0&pageRowNum=1000&needpagecount=true&gridcode=func-salesstquery-grid&queryType=query&dataSource=salesstqueryquery&stagetype=SALESFLOWS&stageid=5&querymoduleid=FuncSalesstqueryNorQuery&sumfieldnames=goodsqty&oper_length=0&page=1&start=0&limit=25'    #改时间
    g=session.post(url,data = date,headers = headers)
    loads = demjson.decode(g.text)   #loads = json.loads(title)           #.decode() 函数解码 JSON 数据。该函数返回 Python 字段的数据类型,即为 u'data'
    pl_htmls=json.dumps(loads, indent=4, sort_keys=False, ensure_ascii=False)     #将 Python 对象编码成 JSON 字符串
    pl = json.loads(pl_htmls)       #将json转化为字典
...
def seles_dx_kc(pl,email):
    ...
    for row in pl["rows"][0:-1]:   #[0:-1]除去最后合计项
       ...
       # print  customname,goodsname, goodstype,salesdate, goodsqty, goodsunit, lotno,unitprice
       # print '--------------------------------------'
        rows=u'欣特'+','+goodsname+','+goodstype+','+ goodsqty+','+ goodsunit+','+ lotno+'\n'
        try:
           with open('./kc/'+nemes,'a')as f:         #保存最后爬取的信息
               s=str(rows)
               f.write(s)
