...
def get_captcha():
    ...
    captcha_url = 'http://czlvpn.oicp.net:56665/ecs/verifyimag.jsp'
...
def login(email, password, captcha):
    ...
    login_url = 'http://czlvpn.oicp.net:56665/ecs/loginactionservlet'
...
'oper_length':'4'
    ...
    url = 'http://czlvpn.oicp.net:56665/ecs/extjsgridQueryServlet/query'
...
def seles_dx(pl,email):
    ...
    for row in pl["rows"][0:-1]:   #[0:-1]除去最后合计项
       ...
       # print '--------------------------------------'
        rows=customname+','+goodsname+','+goodstype+','+certdate+','+ goodsqty+','+ goodsunit+','+ lotno+','+salprice+','+u'采芝林'+'\n'
        try:
           with open('./lx/'+nemes,'a')as f:         #保存最后爬取的信息
               s=str(rows)
               f.write(s)
...
def seles_dx_kc(pl,email):
    ...
    for row in pl["rows"][0:-1]:   #[0:-1]除去最后合计项
       ...
       # print  customname,goodsname, goodstype,certdate, goodsqty, goodsunit, lotno,salprice
       # print '--------------------------------------'
        rows=u'采芝林'+','+goodsname+','+goodstype+','+ goodsqty+','+ goodsunit+','+ lotno+'\n'
        try:
           with open('./kc/'+nemes,'a')as f:         #保存最后爬取的信息
               s=str(rows)
               f.write(s)
