...
def login(email, password):
    login_url='http://flowto.succhi.com:8000/FlowToYTW/Login.aspx'
    #实例化出一个Firefox浏览器
    ...
    driver.find_element_by_id('txt_username').clear()             #使用 clear 方法去清除input或者textarea元素中的内容
    driver.find_element_by_id('txt_username').send_keys(email)
  ...
  #  driver.find_element_by_id('submit').click()    # 选择click函数还是submit函数。推荐每个都试一下，总会有一个成功的。
    print '联合亚太%s销售流向查询'%(email)
    print '联合亚太%s库存查询'%(email)
 #   exp(driver)
