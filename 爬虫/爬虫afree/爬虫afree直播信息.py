...
def bj_url(name,id,sex):       #获取需要爬虫的url
      ...
      if sex=='F':
         ...
         bj_info(url)
         time.sleep(1)
         htmls=requests.get(url,  headers = header).text   #用post结果相同
...
def bj_info(url):   #获取主播个人信息
     ...
     bj_info=docs.xpath('//p[@class="info_bs"]/text()')[0]
     ...
     bj_last_up=docs.xpath('//div[@class="my_bs_guest"]/ul/li[3]/span/text()')[0]
      #---------------------------保存主播信息----------------------------------------
     bj=bj_name.encode('utf-8')+','+bj_id+','+bj_info.encode('utf-8')+','+bj_follower+','+bj_stime+','+bj_swatch+','+bj_last_time+','+bj_last_visit+','+bj_last_up+','+img+'\n'
     print '主播信息：',bj
     try:
...
def bj_img(url_0,img_id):    #bj主页图片信息
     ...
     path="D:\\meinv\\afreetv\\img\\%s"%(img_id)  #用%r，否则print "图片%s成功保存在：%s"%(file_name,path) 解析出错，或者path.encode('utf-8')人，或%r%(file_name,path)
...
if __name__ == '__main__':
   # bj_names=raw_input('爬取的主播：\n')
    count=0
    try:
      bj='bj_name'+','+'bj_id'+','+'bj_info'+','+'bj_follower'+','+'bj_stime'+','+'bj_swatch'+','+'bj_last_time'+','+'bj_last_visit'+','+'bj_last_up'+','+'img'+'\n'
      print '获取数据的字段为',bj
