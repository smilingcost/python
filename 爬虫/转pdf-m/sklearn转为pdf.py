...
def parse_url_to_html(url, name):
    ...
    try:
        ...
        # 正文
        body = soup.find_all(class_="contentLayout2")[0]
        # 标题
...
def get_url_list():
    ...
    :return:
    """
    response = requests.get("http://cwiki.apachecn.org/pages/viewpage.action?pageId=10030181")
    soup = BeautifulSoup(response.content, "html.parser")
    menu_tag = soup.find_all(class_="childpages-macro conf-macro output-block")[0]    #查找相应模块标签的代码，返回一个列表，[0]为提取列表的值，
    print menu_tag
    urls = []                                              #提取后的格式应该为网页html模式，否则无法下面无法识别
    for li in menu_tag.find_all("li"):
        url = "http://cwiki.apachecn.org" + li.a.get('href')       #单个url
