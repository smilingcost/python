...
def get_url_list():
    ...
    :return:
    """
    response = requests.get("http://www.usyiyi.cn/documents/Pandas_0j2/index.html")
    soup = BeautifulSoup(response.content, "html.parser")
    menu_tag = soup.find_all(class_="sphinxsidebar")[0]    #查找相应模块标签的代码，返回一个列表，[0]为提取列表的值，
    print menu_tag
    urls = []                                              #提取后的格式应该为网页html模式，否则无法下面无法识别
    for li in menu_tag.find_all("li"):
        url = "http://www.usyiyi.cn/documents/Pandas_0j2/" + li.a.get('href')       #单个url
